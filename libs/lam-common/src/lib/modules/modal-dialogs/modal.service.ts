...
export class ModalService {
   ...
   */
  select<V = string>(
    options: string[] | SelectOption<V>[], value: V, title: string
  ): Observable<V> {
    return this.custom<SelectionDialogComponent, SelectionDialogComponentData<V>, V>(
      SelectionDialogComponent, { title, options, value }).pipe(
        filter(result => !!result)); // pass value only if a selection was made
  }
  ...
  confirm(
    ...
    return this.custom<ConfirmationDialogComponent, ConfirmationDialogComponentData, boolean>(
      ConfirmationDialogComponent, { title, message, confirmingLabel, dismissiveLabel });
  }
   ...
   */
  message(
    message: string | string[], title = 'Message', closeButtonLabel = 'OK'
  ): Observable<boolean> {
    const messages = Array.isArray(message) ? message : [message];
    return this.custom<MessageDialogComponent, MessageDialogComponentData, boolean>(
      MessageDialogComponent, { title, messages, closeButtonLabel });
  }
   ...
   */
  details(
    message: string, details?: any, title = 'Message',
    yesNo: boolean = false
  ): Observable<boolean> {
    return this.custom<DetailsDialogComponent, DetailsDialogComponentData, boolean>(
      DetailsDialogComponent, { title, message, details, yesNo });
  }
   ...
   */
  input(
    message: string, title = 'Input', initialValue?: string
  ): Observable<string> {
  ...
  custom<T, D, R>(
    ...
    const modal = this.modal.open<T, D>(
      content,
      { hasBackdrop: true, disableClose: true, data }
    );

    return modal.afterClosed().pipe(
      catchError(_ => of(undefined))); // handle promise rejection from Esc key
  }
}
