...
import { toValueOfControl, startsWith } from './rx-utils';
...
describe('RxJS utils', () => {
  ...
  it('toValueOfControl should send correct values thru the observable', async () => {
    ...
    const result = await toValueOfControl(group, 'ctrl').pipe(take(2), toArray()).toPromise();
    expect(result).toEqual(['a', 'b']);
  });

  it('startsWith should be an operator that emits true when a prefixes matches', async () => {
    const source = from(['abc', 'bcd', 'cde', 'def']);
    const result = await source.pipe(startsWith(['ab', 'bc']), toArray()).toPromise();
