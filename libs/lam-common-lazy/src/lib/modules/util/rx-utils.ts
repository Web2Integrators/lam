...
/**
 * Convenience RxJS operator that filters out falsy values and modifies the type accordingly.
 */
export function truthy<T>(): OperatorFunction<T | undefined | null, T> {
...
/**
 * Get an observable for the latest validity of a control including its initial validity.
 *
 * @param source The AbstractControl itself or its ancestor in the hierarchy.
 * @param path If included, the path from source to the control. If not included, then source is
 * used directly.
 */
export function toValidityOfControl(
  source: AbstractControl,
  path?: string | (string | number)[],
): Observable<boolean> {
  const control = path ? source.get(path) : source;
  ...
  // startWith evaluates immediately, but we need to get the validity of the control at subscription
  // time. Deferring allows us to get the updated validity.
  return merge(control.statusChanges, defer(() => of(undefined))).pipe(
    map(_ => control.valid),
    distinctUntilChanged(isEqual),
    shareReplay(1),
  );
